...
import { FiDownload, FiPackage, FiClock, FiCheckCircle, FiMapPin, FiChevronDown, FiChevronUp, FiShoppingBag, FiTruck, FiUser, FiRefreshCw, FiAlertCircle, FiCamera, FiCheck, FiX } from 'react-icons/fi';
...
const MyOrdersPage = () => {
    ...
    const handleReturnSubmit = async () => {
        const selectedItems = selectedSubOrder.items.filter(item => returnItems[item.productId]);
                    ...
                    <div className="space-y-6">
                        {returns.length === 0 ? (
                            <div className="fb-card rounded-lg shadow p-12 text-center">
                                ...
                                <h2 className="text-2xl font-semibold fb-text mb-2">No Returns Requested</h2>
                                <p className="fb-text-secondary">If you have issues with an order, you can request a return here.</p>
                            </div>
                        ) : (
                            returns.map(ret => (
                                            ...
                                            <div className="space-y-4 mb-4">
                                                {parentOrder.subOrders?.map((subOrder, index) => {
                                                    const canReturn = (subOrder.status === 'delivered' || subOrder.deliveryStatus === 'DELIVERED') &&
                                                        (new Date() - new Date(subOrder.deliveryDate || subOrder.updatedAt)) / (1000 * 60 * 60 * 24) <= 2;
