...
const ProfileSettings = () => {
  ...
  if (loading) {
    return (
      <div className="min-h-screen fb-bg flex items-center justify-center">
        <div className="animate-spin rounded-full h-12 w-12 border-b-2 fb-border-primary"></div>
      </div>
    );
  }
  ...
  if (!userData) {
    return (
      <div className="min-h-screen fb-bg flex items-center justify-center">
        <div className="fb-text text-center">
          <h2 className="text-2xl font-bold mb-4">Please log in to view profile settings</h2>
          <Link to="/login" className="fb-btn-primary px-6 py-2 rounded-lg transition-colors">
            Go to Login
          </Link>
        </div>
      </div>
    ...
    <div className="min-h-screen fb-bg py-8 px-4">
      <div className="max-w-4xl mx-auto">
        ...
        <div className="flex items-center mb-8">
          <Link
            to="/"
            className="flex items-center fb-text-primary hover:fb-text-secondary transition-colors mr-4"
          >
            <FaArrowLeft className="mr-2" />
            Back to Home
          </Link>
          <h1 className="text-3xl font-bold fb-text">Profile Settings</h1>
        </div>

        {/* Success Message */}
        {success && (
          <div className="mb-6 p-4 fb-primary-subtle border fb-border-primary rounded-lg fb-text-primary">
            {success}
          </div>
        )}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2 space-y-6">
            <div className="fb-card rounded-xl p-6">
              <h2 className="text-xl font-semibold fb-text mb-6 flex items-center">
                <FaUser className="mr-3 fb-text-primary" />
                Personal Information
              </h2>

              {/* Name Field */}
              <div className="mb-6">
                <label className="block fb-text-secondary mb-2">Full Name</label>
                  ...
                  {editingField === 'name' ? (
                    <div className="flex-1 flex items-center space-x-2">
                      <input
                        ...
                        className="flex-1 fb-surface border fb-border rounded-lg px-4 py-2 fb-text focus:fb-border-primary focus:outline-none"
                      />
                      <button
                    ...
                    <div className="flex-1 flex items-center justify-between">
                      <span className="fb-text">{userData.name}</span>
                      <button
              ...
              {/* Email Field */}
              <div className="mb-6">
                <label className="block fb-text-secondary mb-2">Email Address</label>
                <div className="flex items-center">
                  {editingField === 'email' ? (
                    <div className="flex-1 flex items-center space-x-2">
                      <input
                    ...
                    <div className="flex-1 flex items-center justify-between">
                      <span className="fb-text">{userData.email}</span>
                      <button
              ...
              {/* Phone Field */}
              <div className="mb-6">
                <label className="block fb-text-secondary mb-2">Phone Number</label>
                <div className="flex items-center">
                  {editingField === 'phone' ? (
                    <div className="flex-1 flex items-center space-x-2">
                      <input
                    ...
                    <div className="flex-1 flex items-center justify-between">
                      <span className="fb-text">{userData.phone || 'Not provided'}</span>
                      <button
              ...
              <div className="mb-6">
                ...
                <div className="flex items-center fb-text">
                  <FaCalendarAlt className="mr-3 fb-text-primary" />
            ...
            <div className="fb-card rounded-xl p-6">
              <div className="flex items-center justify-between mb-4">
                <h2 className="text-xl font-semibold fb-text flex items-center">
                  <FaLock className="mr-3 fb-text-primary" />
                  Security
                </h2>
                <div className="flex space-x-2">
                  <button
                    onClick={() => setShowPasswordForm(!showPasswordForm)}
                    className="fb-btn-primary px-4 py-2 rounded-lg transition-colors"
                  >
                    Change Password
                  </button>
                  <button
                    ...
                    className="fb-bg-surface-alt hover:fb-bg-surface-alt/80 fb-text-secondary px-4 py-2 rounded-lg transition-colors border fb-border"
                  >
              ...
              {showPasswordForm && (
                <form onSubmit={handlePasswordChange} className="space-y-4">
                  <div>
                    <label className="block fb-text-secondary mb-2">Current Password</label>
                    <div className="relative">
                      <input
                        type={showCurrentPassword ? 'text' : 'password'}
                        value={passwordForm.currentPassword}
                        onChange={(e) => setPasswordForm({ ...passwordForm, currentPassword: e.target.value })}
                        className="w-full fb-surface border fb-border rounded-lg px-4 py-2 fb-text focus:fb-border-primary focus:outline-none pr-12"
                        required
                      />
                      <button
                        type="button"
                        onClick={() => setShowCurrentPassword(!showCurrentPassword)}
                        className="absolute right-3 top-1/2 transform -translate-y-1/2 fb-text-muted hover:fb-text-secondary"
                      >
                  ...
                  <div>
                    <label className="block fb-text-secondary mb-2">New Password</label>
                    <div className="relative">
                      <input
                  ...
                  <div>
                    <label className="block fb-text-secondary mb-2">Confirm New Password</label>
                    <input
                      type="password"
                      value={passwordForm.confirmPassword}
                      onChange={(e) => setPasswordForm({ ...passwordForm, confirmPassword: e.target.value })}
                      className="w-full fb-surface border fb-border rounded-lg px-4 py-2 fb-text focus:fb-border-primary focus:outline-none"
                      required
                    />
                  </div>
                  ...
                  <div className="flex space-x-4">
                    <button
                      type="submit"
                      className="fb-btn-primary px-6 py-2 rounded-lg transition-colors"
                    >
                      Update Password
                    </button>
                    <button
                      ...
                      className="fb-bg-surface-alt hover:fb-bg-surface-alt/80 fb-text px-6 py-2 rounded-lg transition-colors border fb-border"
                    >
                      Cancel
                    </button>
                  </div>
          ...
          <div className="space-y-6">
            {/* Order Statistics */}
            <div className="fb-card rounded-xl p-6">
              <h3 className="text-lg font-semibold fb-text mb-4 flex items-center">
                <FaShoppingBag className="mr-3 fb-text-primary" />
                Order Statistics
              </h3>
              <div className="space-y-3">
                <div className="flex justify-between">
                  <span className="fb-text-secondary">Total Orders:</span>
                  <span className="fb-text-primary font-semibold">{orderStats.totalOrders}</span>
                </div>
                <div className="flex justify-between">
                  <span className="fb-text-secondary">Completed:</span>
                  <span className="text-green-500 font-semibold">{orderStats.completedOrders}</span>
                </div>
                <div className="flex justify-between">
                  <span className="fb-text-secondary">Last Order:</span>
                  <span className="fb-text text-sm">{orderStats.lastOrderDate}</span>
                </div>
              </div>
              <Link
                to="/myorders"
                className="block w-full fb-btn-primary text-center py-2 rounded-lg transition-colors mt-4"
              >
                View All Orders
              </Link>
            </div>
            ...
            <div className="fb-card rounded-xl p-6">
              <button
                ...
                className="w-full flex items-center justify-between fb-text text-lg font-semibold mb-4"
              >
                <div className="flex items-center">
                  <FaQuestionCircle className="mr-3 fb-text-primary" />
                  FAQs
                </div>
              ...
              {showFAQ && (
                <div className="space-y-4 text-sm">
                  <div>
                    <h4 className="fb-text-primary font-semibold mb-2">
                      What happens when I update my email address?
                    </h4>
                    <p className="fb-text-secondary leading-relaxed">
